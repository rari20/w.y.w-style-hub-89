...
import { customers, riskColors } from '@/data/adminData';
...
export default function AdminChurnRisk() {
  ...
  const exportCSV = () => {
    const header = 'ID,Name,Email,Tier,TotalSpend,DaysInactive,ChurnRisk,Satisfaction';
    const rows = atRisk.map(c => `${c.id},${c.name},${c.email},${c.loyaltyTier},${c.totalSpend},${c.daysInactive},${c.churnRisk},${c.satisfaction}`);
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-display">High Churn Risk Customers</h1>
            <p className="text-xs text-muted-foreground font-body">Customers identified by the W.Y.W CART model as likely to churn. Root split: Total Spend 6M &lt; £500.</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {atRisk.map(c => {
            ...
            const pills = [
              ...
              { label: 'Email disengaged', color: 'bg-red-500/15 text-red-700 dark:text-red-400' },
              { label: 'No consultations', color: 'bg-amber-500/15 text-amber-700 dark:text-amber-400' },
              ...
              <Card key={c.id}>
                <CardContent className="p-5 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button size="sm" variant="outline" className="text-xs gap-1.5" onClick={() => act(c.id, 'discount', `20% discount sent to ${c.name}`)}>
                      <Tag className="h-3.5 w-3.5" strokeWidth={1.5} /> Send 20% Discount
                    </Button>
                    <Button size="sm" variant="outline" className="text-xs gap-1.5" onClick={() => act(c.id, 'consultation', `Free consultation offered to ${c.name}`)}>
                      <Calendar className="h-3.5 w-3.5" strokeWidth={1.5} /> Offer Free Consultation
                    </Button>
                    <Button size="sm" variant="outline" className="text-xs gap-1.5" onClick={() => toast.success(`${c.name} tier upgraded.`)}>
